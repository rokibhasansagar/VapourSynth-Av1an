...
RUN --mount=type=secret,id=RCLONE_CONFIG_HASH <<-'EOL'
	...
	(
	  ...
	  export RCLONE_CONFIG_HASH=$(< /run/secrets/RCLONE_CONFIG_HASH)
	  ...
	  find /var/cache/pacman/pkg/ -depth -type f -name "*-debug-*.pkg.tar.zst" -exec sudo rm -vf '{}' + 2>/dev/null
	  rclone copy /var/cache/pacman/pkg/ --include="*.pkg.tar.zst*" "ms365:Public/TestArchBuilds/${rclarch}/" --fast-list 2>/dev/null
